...
def adams_comp_pend(filename, adams_file_path=None, plot_diff=False):
    ...
    for b in range(n_bodies):
        ...
        for i in range(dofs_per_body):
            ...
            axis_str = f" {axis_label}" if axis_label else ""
        ...
        if len(adams_data) > 0:
            ...
            if col_idx < adams_data.shape[1]:
                if np.any(np.abs(adams_data[:, col_idx]) > 1e-10):
                    ...
                    label_suffix = f" {active_axis}" if active_axis and active_axis != 'unknown' else ""
        ...
        if b == 0:
            ax.legend(loc='upper right', fontsize=14, frameon=True, framealpha=0.9, labelspacing=1.2)
    ...
    plt.tight_layout()
    plt.subplots_adjust(bottom=0.08) # Ensures the x-axis label is not cut off at the bottom
    plt.show()

### CONFIG ###
filename1 = "5p_energies.csv"
filename2 = "5p_energies_half.csv"
filename3 = "5p_gen_acc.csv"

# plot_all_energies(filename1)

# compare_TE(filename1,filename2)

adams_path = r"C:\Users\kaspe\OneDrive - Aarhus universitet\6. semester\Bachelorprojekt\Adams\5p_adams_results_SI2.csv"

# adams_comp_pend(filename3, adams_path, plot_diff=True)
